...
import (
	...
	"github.com/wirelineio/wire-ns/app"
)
...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "wnsd",
		Short:             "WNS Chain App Daemon (server)",
	...
	executor := cli.PrepareBaseCmd(rootCmd, "WNS", app.DefaultNodeHome)
...
func exportAppStateAndTMValidators(
	...
	wnsChain := app.NewAppInit(logger, db, traceStore, true, map[int64]bool{}, 0)

	if height != -1 {
		err := wnsChain.LoadHeight(height)
		if err != nil {
			return nil, nil, err
		}
	}

	return wnsChain.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}
