...
function init_config ()
{
  ...
  wnscli config keyring-backend $KEYRING_BACKEND
...
function init_root ()
{
  ...
  echo -e "${MNEMONIC}\n${PASSPHRASE}\n${PASSPHRASE}" | wnscli keys add root --recover
  echo -e "${PASSPHRASE}" | wnsd add-genesis-account $(wnscli keys show root -a) 100000000000000uwire

  # Validator stake/bond => 10 million WIRE (out of total 100 million WIRE).
  echo -e "${PASSPHRASE}\n${PASSPHRASE}\n${PASSPHRASE}" | wnsd gentx --name root --amount 10000000000000uwire --keyring-backend $KEYRING_BACKEND --home-client "${WNS_CLI_CONFIG_DIR}"
  wnsd collect-gentxs
